...
def compute_kappa_and_gradient_full(T,rho_dep,direction,N):
...
def fourier(**options):
    ...
    def func(rho):
     ...
     for n,direction in enumerate(directions):
      ...
      rho_dep = [kappa_ij,P_vec,i_mat,j_mat,ind_extremes,kappad_ij,kappad_ji]

      kappa_and_gradient =  partial(compute_kappa_and_gradient_full,rho_dep=rho_dep,N=N,direction=direction)
...
def get_optimizer():
 ...
 def optimize(*kd):
  ...
  def func(x,grad,beta):
   ...
   #grad[:]  += 1/g*((kappa[1]-kd[1])*jacobian[0]  + (kappa[1]-kd[1])*jacobian[1]  + (kappa[2]-kd[2])*jacobian[2] )
   return g
